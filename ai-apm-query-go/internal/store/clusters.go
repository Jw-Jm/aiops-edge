...
func (d *ClusterDAO) List() ([]Cluster, error) {
	...
	rows, err := conn.Query(
		"SELECT id, name, provider, region, version, node_count, status, api_server, created_at, updated_at FROM clusters ORDER BY name")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Name, &c.Provider, &c.Region, &c.Version,
			&c.NodeCount, &c.Status, &c.APIServer, &c.CreatedAt, &c.UpdatedAt); err != nil {
			return nil, err
		}
...
func (d *ClusterDAO) GetByName(name string) (*Cluster, error) {
	...
	row := conn.QueryRow(
		"SELECT id, name, provider, region, version, node_count, status, api_server, created_at, updated_at FROM clusters WHERE name = ?",
		name)
	var c Cluster
...
func (d *ClusterDAO) Upsert(c *Cluster) (int64, error) {
	...
	res, err := conn.Exec(
		"INSERT INTO clusters (name, provider, region, version, node_count, status, api_server) VALUES (?, ?, ?, ?, ?, ?, ?) "+
			"ON DUPLICATE KEY UPDATE provider=?, region=?, version=?, node_count=?, status=?, api_server=?",
		c.Name, c.Provider, c.Region, c.Version, c.NodeCount, c.Status, c.APIServer,
		c.Provider, c.Region, c.Version, c.NodeCount, c.Status, c.APIServer)
	if err != nil {
		return 0, err
	}
