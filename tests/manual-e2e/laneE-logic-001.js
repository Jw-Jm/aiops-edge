...
async function run() {
  ...
  await withSession({ viewport: vp, col, seedScope: true }, async (page) => {
    ...
    check('server_session_revoked_after_ui_logout', meAfterLogout.status() !== 200,
      `UI 退出后 GET /me -> ${meAfterLogout.status()} ${meBody.slice(0, 120)}`)
    ...
    check('api_wrong_current_password_rejected', badPw.status() === 400 && /invalid_current_password/.test(badPwBody),
    ...
    if (n >= 3) {
      ...
      const rejected = /失败|错误|不正确|当前密码|incorrect|wrong/i.test(bodyText)
      check('ui_wrong_current_password_rejected', rejected,
        `提交后 url=${urlAfter}；页面提示=${bodyText.slice(0, 120)}`)
      check('ui_wrong_password_no_forced_logout', !/\/login/.test(urlAfter),
        `提交错误当前密码后 url=${urlAfter}`)
    } else {
