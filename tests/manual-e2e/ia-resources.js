...
async function run() {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    await withSession({ viewport, col }, async (page) => {
      ...
      check('panorama_visible', await page.getByTestId('cluster-panorama').isVisible().catch(() => false))
      ...
      check('resource_directory_or_empty_state', await page.getByRole('region', { name: '资源目录' }).count().then((count) => count > 0).catch(() => false) || await page.getByText(/当前域暂无资源|资源目录读取失败/).count().then((count) => count > 0).catch(() => false))
