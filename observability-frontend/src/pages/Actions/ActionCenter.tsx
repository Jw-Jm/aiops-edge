...
// 时间轴只消费已本地化的 view 字段，避免出现 approved/pending 等裸英文状态。
const lifecycle = (view: ReturnType<typeof toActionViewModel>) => [
  { label: '提出', value: '已提出' },
  { label: '预检', value: view.preflightStatus },
  { label: '审批', value: view.approvalStatus },
  { label: '执行', value: view.executionStatus },
  { label: '验证', value: view.verificationStatus },
  { label: '回滚', value: view.rollback },
]
...
const ActionCenter: React.FC = () => {
  ...
  // 首屏固定六列：操作/目标、风险、审批、执行、验证、操作。
  // 来源 Run、影响范围与创建时间进入详情 Drawer，不在首屏占用横向空间。
  const columns = useMemo(() => [
    { title: '操作 / 目标', key: 'target', width: 260, render: (_: unknown, row: ActionProjection) => { const view = toActionViewModel(row); return <div><Typography.Text strong>{row.operation || row.action_type}</Typography.Text><div className="cell-wrap" style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{view.target}</div></div> } },
    { title: '风险', key: 'risk', width: 110, responsive: ['xl'] as ('xl')[], render: (_: unknown, row: ActionProjection) => <Tag color="gold">{toActionViewModel(row).risk.label}</Tag> },
    { title: '审批', key: 'approval', width: 110, render: (_: unknown, row: ActionProjection) => { const view = toActionViewModel(row); return <Tag color={view.approvalStatus === '已批准' ? 'green' : view.approvalStatus === '已拒绝' ? 'red' : 'gold'}>{view.approvalStatus}</Tag> } },
    { title: '执行', key: 'execution', width: 120, render: (_: unknown, row: ActionProjection) => <span className={toActionViewModel(row).executionStatus === '执行中' ? 'flow' : undefined}>{toActionViewModel(row).executionStatus}</span> },
    { title: '验证', key: 'verification', width: 110, responsive: ['xl'] as ('xl')[], render: (_: unknown, row: ActionProjection) => toActionViewModel(row).verificationStatus },
    { title: '操作', key: 'actions', width: 96, render: (_: unknown, row: ActionProjection) => <Button type="link" size="small" onClick={(event) => { event.stopPropagation(); setSelected(row) }}>查看详情</Button> },
  ], [])
  ...
  return <div data-testid="action-center">
    ...
    <Tabs defaultActiveKey="proposed" items={actionTabs.map((tab) => ({ key: tab.key, label: `${tab.label} (${filteredActions(tab.key).length})`, children: <Table rowKey="action_id" loading={loading} tableLayout="fixed" columns={columns} dataSource={filteredActions(tab.key)} pagination={{ pageSize: 20 }} scroll={{ x: 'max-content' }} locale={{ emptyText: <Empty description="暂无动作" /> }} onRow={(row) => ({ onClick: () => setSelected(row) })} /> }))} destroyOnHidden />
    <Drawer open={!!selected} width={620} title="动作详情" onClose={() => setSelected(null)} extra={<Button onClick={refreshDetail}>刷新</Button>}>
      {selected && <>
        ...
        <Descriptions size="small" column={1} style={{ marginTop: 16 }} items={[
          { key: 'impact', label: '影响范围', children: toActionViewModel(selected).impact },
          { key: 'created', label: '创建时间', children: selected.created_at ? selected.created_at.slice(0, 19).replace('T', ' ') : '未提供' },
          { key: 'approval', label: '审批状态', children: toActionViewModel(selected).approvalStatus },
          { key: 'execution', label: '执行状态', children: toActionViewModel(selected).executionStatus },
          { key: 'verification', label: '验证状态', children: toActionViewModel(selected).verificationStatus },
        ...
        <Timeline style={{ marginTop: 24 }} items={lifecycle(toActionViewModel(selected)).map((step) => ({ children: <Space><Typography.Text strong>{step.label}</Typography.Text><Tag className={step.value === '执行中' ? 'flow' : undefined}>{step.value}</Tag></Space> }))} />
