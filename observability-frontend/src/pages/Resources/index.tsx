...
const Resources: React.FC = () => {
  ...
  const views = [
    { key: 'containers', label: '容器资源', children: <ResourceDirectory clusterId={activeClusterId} group="containers" selected={active.resource} onSelect={selectResource} /> },
    { key: 'kubevirt', label: 'KubeVirt 虚拟机', children: <ResourceDirectory clusterId={activeClusterId} group="kubevirt" selected={active.resource} onSelect={selectResource} /> },
  ...
  return (
    <div>
      ...
      <ResourceCenter><Tabs activeKey={activeKey} items={views} onChange={(key) => setParams((current) => { current.delete('view'); if (key === 'relationships') current.set('view', 'graph'); else { current.set('group', key); current.delete('type') }; return current })} destroyOnHidden /></ResourceCenter>
    </div>
  )
}
