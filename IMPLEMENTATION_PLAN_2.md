...
> 定位：在 **192.168.0.63 单节点 K8s 真实环境**上，将「AIOps 平台构建方案」落地为可执行的完整实施指南。
> 本文档 = **环境实际情况 + 部署以来全部问题（经验固化）+ 平台四维构建实施** 的整合，是本次构建的唯一可执行依据。
...
### 3.1 目标
在现有平台上，从**界面 / 功能 / 框架 / 架构**四维打磨产品体验，且**全部自研**（独立实现，不复制任何外部产品代码）。

### 3.2 合规红线
- 借鉴业界优秀的界面设计、功能清单、架构思想，自主设计实现。
- **严禁复制**任何第三方代码文件（Go/TSX/CSS/脚本）。
- 参考源仅用于阅读理解，不落地到本仓库。
...
**目标**：引入 Zustand；深色 token 收敛为统一色板；Sidebar 重构为 7 分区；新增页用 Tailwind。
**改动**：
...
| 风险 | 说明 | 应对 |
|---|---|---|
| 合规 | 第三方参考可能含开源许可 | 全自研，仅借鉴设计/功能/架构思想 |
