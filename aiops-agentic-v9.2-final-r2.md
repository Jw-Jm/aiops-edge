...
## V9.2 FINAL R2 — Consolidated Execution Contract
...
> **R2 整理日期**：2026-08-20
> **R2 整理范围**：合并 V9.2 原始合同、Phase 2/3 Gate 结论、Phase 3 安全勘误、Phase 4 R3 计划及 P4.3/P4.4 cutover 决策

---

# 零、R2 文档地位、决策优先级与当前状态

## 0.1 文档地位

本文件是 `aiops-agentic-v9.2-final.md` 的整理后继版本。它保留原 V9.2 的完整目标架构、Phase 1–21、Gate、测试纪律和 Definition of Done，并把原文形成之后已经明确拍板或通过真实实施验证的结论并入正文。

从 R2 开始，后续实施按以下优先级解释冲突：

```text
1. 用户最新明确拍板的决定
2. 本 R2 中标记为 FROZEN / APPROVED / PASS 的条款
3. 已通过测试和 Gate 的真实生产代码行为
4. 当前 Phase 的经批准实施计划
5. 原 V9.2 未被修订的条款
6. 更早的 V1–V9.1 草稿、讨论稿和过渡建议
```

如果低优先级材料与高优先级结论冲突，必须执行高优先级结论，不得以“原文如此”为由恢复已废弃设计。
...
P3.1–P3.10c-final   COMPLETE
P3.11                PASS
P3.12                COMPLETE
Phase 3 P0 defects   0

P4.3 migrator framework                 COMPLETE
P4.3 schema-migrator executable         COMPLETE
P4.3 checksum / lock / idempotency      COMPLETE
P4.3 RequireCurrentVersion              COMPLETE
P4.3 query-api runtime cutover           DEFERRED_TO_P4.4
```

Phase 3 的两个剩余 orchestrator 测试：
...
```text
LEGACY_PATH_PENDING_REMOVAL
planned_removal_phase = 14
```

它们不是 Phase 3 production defect，不允许为追求当前全绿而在 Phase 4 修改旧业务路径。

## 0.4 当前唯一允许的下一步

当前只能继续 Phase 4，不得跳到 Phase 5。P4.4 必须先完成：

```text
现有 EnsureSchema 行为完整盘点
→ 全部 runtime-required DDL 进入 versioned migrations
→ schema backfill DML 进入 versioned migrations
→ 合法 seed 拆为 DML-only bootstrap
→ migration coverage test PASS
→ existing environment 迁移完成
→ query-api 切换到 RequireCurrentVersion
→ query-api startup zero DDL 得证
```

只有完整 Gate 4 PASS，才允许进入 Phase 5 writer cutover。
...
**当前状态：PASS。P3.1–P3.10c-final COMPLETE，P3.11 PASS，P3.12 COMPLETE，Phase 3 P0 defect = 0。**
...
ServiceAuthenticator
EdDSA / Ed25519 JWS signing
nonce replay defense

Resource Resolver
Kubernetes Access Boundary
Cluster Credential Identity Binding
```
...
Phase 3 已完成的应用级多 Cluster 闭环必须保持：

```text
canonical cluster UUID
→ Cluster Registry
→ tenant_clusters ownership
→ credential_ref
→ Kubernetes Access Boundary
→ SecretResolver
→ kubeconfig
→ kube-system UID identity validation
→ ClusterClientManager
→ correct Kubernetes API
```

错误映射必须在边界内部 fail closed：

```text
UUID_A + credential_B
→ observed kube-system UID != registry binding
...
```text
two canonical UUIDs                          PASS
tenant ownership                             PASS
independent credential_ref                   PASS
same-name cross-cluster resource isolation  PASS
canonical UUID Kubernetes routing            PASS
missing Secret fail-closed                   PASS
credential mismatch fail-closed              PASS
default/all/numeric/current-context fallback ABSENT
```
...
本 Gate 已 PASS。后续 Phase 不得绕过 `internal/k8sboundary` 新增直接 kubeconfig、current-context 或裸 Kubernetes Client 路径。

---

# 七十、Phase 4：新数据模型与统一初始化

**当前状态：IN_PROGRESS。P4.3 migrator framework 已完成；query-api runtime cutover 按批准的依赖调整移动到 P4.4 final substep。**

建立新结构。

**不物理删除旧历史数据。**

统一 versioned Schema Init。

运行服务账号不允许 CREATE TABLE。

必须建立 Schema Version，并对 migration id 与 checksum 双校验。

DDL authority 固定为：

```text
MySQL
schema-migrator + aiops_migrator

ClickHouse
versioned bootstrap Job + dedicated migration account

Chroma / MinIO
bootstrap/init Job

runtime services
read-only schema compatibility/readiness check only
```
...
禁止复用或改造旧 `schema_migrations`；旧表只能标记 `LEGACY_MIGRATION_METADATA`。

`RequireCurrentVersion` 固定语义：
...
Phase 4 只建立 schema、ownership、bootstrap 和 runtime no-DDL 边界。业务 writer cutover 属于 Phase 5，reader/query cutover 属于 Phase 6。
...
```text
P4.3-A  aiops_schema_migrations        COMPLETE
P4.3-B  checksum / lock / idempotency  COMPLETE
P4.3-C  schema-migrator executable     COMPLETE
P4.3-D  RequireCurrentVersion          COMPLETE
P4.3-E  query-api runtime cutover      DEFERRED_TO_P4.4
```

批准理由：旧 `EnsureSchema()` 同时承担 20+ 张表的 DDL、schema evolution、历史 backfill DML 和 bootstrap seed；P4.3 只有 migrator 框架，尚未取得完整 schema authority。此时立即替换会让 fresh/current deployment 在 authoritative migrations 覆盖不足时 fail closed，破坏既有启动能力。
...
SECURITY IMPACT:
No new privilege or fallback is introduced. Existing startup behavior remains
temporarily until complete migration coverage is proven.

CUTOVER DEADLINE:
P4.4 final substep, before entering P4.5.
```

不得把 cutover 继续推迟到 P4.8。P4.4 负责代码层停止 runtime DDL；P4.8 负责用数据库权限强制 runtime 不能执行 DDL。

## 70.2 P4.4 的完整职责

P4.4 不是只创建 `0002_ai_runtime.sql` 和 `0003_platform_audit.sql`。它还必须完整接管现有 `EnsureSchema()` 的 MySQL DDL authority。

必须将每项行为归入且只归入以下一类：

```text
1. DDL
   CREATE TABLE / ALTER TABLE / CREATE INDEX / schema evolution
   → versioned migrations

2. Schema migration/backfill DML
   例如旧数据兼容所需 UPDATE users SET user_uuid = ...
   → versioned migrations
   → 不得每次 runtime startup 执行

3. Legitimate bootstrap seed DML
   初始 admin / role / permission / required system settings
   → store.EnsureBootstrapData()
   → idempotent DML only
   → NO CREATE / ALTER / DROP / INDEX
```
...
```text
1. Inventory current EnsureSchema behavior
2. All DDL → versioned migrations
3. Schema backfill DML → versioned migrations
4. Legitimate startup DML → DML-only bootstrap function
5. Run schema-migrator against existing environment
6. Verify migrations complete and checksums match
7. Replace query-api EnsureSchema() with:
      RequireCurrentVersion()
      + optional EnsureBootstrapData()
8. Prove query-api startup performs zero DDL
```

P4.4 必须增加 migration coverage Gate：

```text
TestMigratedSchemaCoversLegacyEnsureSchema

fresh DB
→ schema-migrator
→ information_schema snapshot A

controlled reference DB
→ legacy EnsureSchema
→ information_schema snapshot B

assert:
A covers every Phase-4 runtime-required table / column / index in B
```

仍被当前 runtime 依赖的 LEGACY 表也必须由 baseline migration 创建；标记 LEGACY 不等于允许 fresh environment 缺表。

## 70.3 Phase 4 冻结的 MySQL 表

AI Runtime / audit 表至少包括：
...
禁止用空字符串表达 default cluster，禁止把多集群压入单个 `cluster_id` 字段。

## 70.4 Phase 4 其他存储边界

```text
VictoriaMetrics
→ label contract，raw metrics SoT

VictoriaLogs
→ label contract，raw logs only SoT

ClickHouse
→ versioned bootstrap + schema compatibility check
→ no runtime CREATE/ALTER/DROP
→ log_records 标 LEGACY，不再写完整 raw log copy

Chroma
→ bootstrap Job 创建/验证 collection
→ runtime get/validate only

MinIO-compatible Object Store
→ bootstrap Job 创建/验证 required buckets
→ large Evidence + Knowledge objects SoT
```

orchestrator 对 V9.2 新 MySQL 表禁止直连写入，只能通过 query-api 内部控制面。旧直连依赖若暂时存在，必须显式标记 `LEGACY_DIRECT_MYSQL_DEPENDENCY`、列出 removal phase，且不得扩张。
...
existing environment upgrade/preservation PASS
required migration id/checksum verification PASS
schema checksum tamper detection PASS
legacy runtime-required schema coverage PASS
query-api startup zero DDL PASS
runtime accounts without DDL permission can start PASS
MySQL users/platform_settings/clusters.credential_ref unchanged
Kubernetes credential Secret objects unchanged
ClickHouse runtime CREATE rejected
Chroma second init idempotent
MinIO bucket bootstrap/validation PASS
raw logs only in VictoriaLogs PASS
```

Secret 验证只允许比较存在性、对象 metadata 或 digest；禁止把 Secret data、token、certificate、private key 或完整 kubeconfig 写入报告。
