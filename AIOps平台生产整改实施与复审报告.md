...
- **验收标准：** evidence JSON 的 `git_commit=865de6a...` 与候选源码提交一致、所有 Pod image digest 与 manifest 一致、`publishable=true`；每个自研镜像由 registry immutable digest 与签名 release manifest 绑定；Query 与 Worker 均从同一候选源码完成重建，回滚到上一 digest 和再次升级均成功；忽略规则只允许明确的本地运行时 artifact，不得隐藏源码或配置。
