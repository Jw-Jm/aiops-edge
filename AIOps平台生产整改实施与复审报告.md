...
| FIX-CHAT-04 | `apps/investigation.py:run_invocation` 的授权矩阵异常将 `str(exc)` 直接作为 403 detail，可能泄漏租户、角色、数据库或内部拓扑信息。 | 签名运行入口仅返回 `stable_error_code(..., "AUTHZ_DENIED")`，原异常通过异常链保留给服务端日志，不跨越 HTTP 边界。 | `tests/test_p19_chat_ingress.py::test_run_invocation_authz_error_does_not_echo_internal_details`；生产架构契约与 AICHAT 回归通过。 |
| FIX-CHAT-05 | `node_verify`、DAG full fallback、shell/K8sGPT/MCP 及旧 Action/报告/NL2SQL/变更兼容出口仍把异常原文回传或持久化；即使生产路由 retired，本地迁移调用也可污染审计/响应。 | 统一使用稳定错误码：`VERIFICATION_SOURCE_UNAVAILABLE`、`BRAIN_ERROR`、`EXECUTION_FAILED`、`K8SGPT_FAILED`、`MCP_TOOL_FAILED`、`*_QUERY_FAILED` 等；不改变生产路由 allowlist 或禁写策略。 | `tests/test_chat_tool_boundary.py` 新增 6 条敏感文本回归；`tests/test_ops_action_hub.py` 状态码契约更新；受影响 Python 隔离全量 **1275 passed**。 |
...
- GitHub：`origin/main` 已包含本轮运行代码与报告提交 `37f77aa9df29a5299feeadab4bdca1db1031cfe7`；工作区无代码修改，用户既有 `.ses` 运行时文件未纳入提交。
- 镜像：`IMAGE_TAG=git-37f77aa` 的 12 个自研镜像全部本机构建成功，OCI revision label 与提交 `37f77aa9df29a5299feeadab4bdca1db1031cfe7` 一致；Docker 构建使用本机缓存基础镜像，无复用旧业务镜像。
- Helm：`aiops` revision **57**，`STATUS=deployed`。自研 Deployment/DaemonSet/Worker/迁移 Job 均使用 `git-37f77aa`；Pod 实际 imageID 已逐一核对，业务 Pod 全部 Ready、迁移及回填 Job 全部 Complete、重启数为 0。
...
| 检查 | 实际结果 | 结论 |
|---|---|---|
| Python 受环境隔离全量 | `1275 passed, 1 skipped, 3 deselected, 2 warnings`；仅排除 `test_uvicorn_protocol_rejects_wrong_san_over_real_tls`、`test_production_full_reachable_returns_remote`、`test_check_control_plane_reachable_reachable`，原因是宿主禁止 localhost 临时监听；单独运行其余网络相关测试均通过。 | **有限通过**；3 项环境限制仍未验证。 |
...
| release evidence | `/tmp/aiops-release-evidence-37f77aa.json` 记录代码提交 `37f77aa9df29a5299feeadab4bdca1db1031cfe7`、工作区 clean、12 个本机镜像 presence/revision、Helm/契约/单测均 PASS；`registry_bound=false`、签名 binding/KMS 公钥及 rendered manifest 缺失，最终 `publishable=false`。 | **按设计阻断** |

### 0.0.4 真实观测与 RCA 结论

- 无 fixture 的 `validate-observability-evidence.sh --output /tmp/aiops-evidence-37f77aa.json` 实际返回 **exit=2 / `gate_status=BLOCKED_BY_ENV`**，原因是未提供 `AIOPS_VALIDATION_DATA_MARKER`；没有把空数据或 fixture 当成生产证据。
...
当前 **不可发布到生产**。已完成的代码级阻断：RCA failed-envelope 误判、硬件事件严重度丢失、Investigation envelope 误读、Chat/Investigation 异常敏感信息泄露（含 Worker 授权、DAG/验证、shell/K8sGPT/MCP 和兼容出口）；并已同步 GitHub、镜像和本机 Helm 运行态。解除发布阻断的最小集合仍为：
...
整改验收顺序：先以 `37f77aa` 重建并核对镜像/Pod → ChatTool 审计与错误边界回放 → 真实 marker 全域观测 → RCA confirmed 闭环 → registry/KMS 与 HA/恢复门禁。任一外部证据缺失均保持 `BLOCKED_BY_ENV` 或 `FAIL`。
...
| `hugegraph_client.go:528+`、`hugegraph_repository.go`：CUSTOMIZE_STRING 顶点的索引边回退 | RCA 邻居查询在 HugeGraph K-neighbor 为空或不支持自定义字符串 ID 时，必须仍能按 tenant/cluster/source 读取有界依赖边 | 真实 K-neighbor 返回空图，但 `graph/edges?vertex_id=<trace uid>` 有边；旧实现直接返回空，RCA 图增强丢失依赖关系 | 新增 `EdgesForVertex` 只读索引端点并在 Repository 构造有界 scoped subgraph；单测通过；本机真实 K-neighbor 零结果/索引边非零对照，最终 RCA Run 图为 3 vertices/2 edges/1 path |
| `topology.go`、`trace.py`：物化边、parent-span/sequence fallback 和 TraceBuilder 过滤自环 | 服务拓扑只能表达跨服务依赖；同一服务的 span/sequence 不得生成 `service -> itself` 边 | 物化 `service_topology` 与 fallback 均被 2874 条自环淹没，TraceBuilder 还会写入自环，导致图查询无法得到跨服务边 | SQL 与 Python builder 均跳过 source==target；Go/Python 定向测试通过；Query 镜像 `git-865de6a` 部署后最终真实 RCA 图出现 2 条跨服务边，active self-loop=0；Worker 镜像重建因基础镜像 EOF 未验证 |
...
| 生产就绪度 | **不通过** | Helm revision 25 的 Query/Graph 修复、Graph reconcile、AICHAT 冒烟、IntentEngine 动态时间窗、Ingest Trace/Event marker 写入和 VM/VLogs/Event/DeepFlow/dependency 真实 marker 读回通过；`collect-release-evidence.sh` 可返回 `working_tree_dirty=false,publishable=true`，但仍无 registry immutable digest/signature；RCA confirmed 合同、生产 Secret、真实 Provider、HA/备份/回滚仍无候选环境证据。 |
...
> **本轮证据勘误：** 下表中早期 revision 19–24 / 旧镜像标签和中间步骤记录来自上一轮复审；本轮当前运行态以新增的 2.3.1 节为准：Query/Graph 功能代码 `865de6a`、Query 镜像 `query-api:git-865de6a`、Helm revision 25。旧 canary 结果仅作为历史代码证据，不冒充本轮 fresh install 的实时观测数据。
...
### 2.3.1 本轮 `865de6a` 代码与运行证据（当前基线）

以下是本轮在 Query/Graph 功能代码 `865de6a` 上重新执行的证据；Query 镜像 `query-api:git-865de6a` 已部署到 Helm revision 25。Worker 的 TraceBuilder 自环过滤源码来自 `8c73f02`，但 Python 基础镜像拉取 EOF 导致该镜像本轮未重建，运行中仍为既有 Worker 镜像。未运行 Graph 压测或 `graph-load-test.sh`；本轮执行了真实 Kubernetes→HugeGraph generation 同步、Query→HugeGraph 索引边回退、真实 marker 全域数据读取和 RCA 图增强验证。Graph 200k/1M 容量门禁沿用已通过且明确不属于压力测试的证据。

| 检查 | 命令与实际结果 | 结论 |
|---|---|---|
| Query Go 静态与单元测试 | `cd ai-apm-query-go && GOCACHE=/tmp/aiops-go-cache go vet ./...`；`GOCACHE=/tmp/aiops-go-cache go test ./...`（授权本机环境） | **通过**；全量 Query 包（含租约 claim/renew、Runtime Commit、Outbox、AICHAT、授权、Graph repository 和 topology fallback）通过。沙箱内曾因 `httptest` IPv6 监听权限失败，授权本机重试后通过。 |
| 其他 Go 服务全量静态与单元测试 | `for d in ai-action-executor ai-credential-broker ai-llm-egress-proxy ai-apm-ingest-go ai-event-collector deploy/tools/object-store-bootstrap deploy/tools/clickhouse-migrator; do (cd "$d" && GOCACHE=/tmp/aiops-go-cache go vet ./... && GOCACHE=/tmp/aiops-go-cache go test ./...); done`（授权本机环境） | **通过**；各 module 的 `go vet` 与 `go test` 均返回 0；沙箱 IPv6 监听限制已通过本机重试解除。 |
| Python 服务全量隔离测试 | `cd ai-orchestrator && AIOPS_DEPLOYMENT_MODE=development AIOPS_ENV=development LLM_MOCK=true .venv314/bin/python -m pytest -q -p no:cacheprovider -k 'not test_uvicorn_protocol_rejects_wrong_san_over_real_tls and not test_production_full_reachable_returns_remote and not test_check_control_plane_reachable_reachable'` | **1229 passed, 1 skipped, 3 deselected, 2 warnings**；3 个需要 localhost/真实网络回环的测试因当前执行环境限制显式排除，不能推定为通过；生产模式启动缺少 `QUERY_API_URL`/签名密钥/内部 token 时按代码 fail-closed，未伪造值。 |
...
| 镜像构建与代码一致性 | `BUILD_IMAGES_DRY_RUN=1 IMAGE_TAG=git-865de6a bash deploy/scripts/build-images.sh query-api`；`IMAGE_TAG=git-8c73f02 bash deploy/scripts/build-images.sh orchestrator`；`docker image inspect`；`kubectl get deploy` | **Query 通过，Worker 未验证**；Query 镜像按 `865de6a` 构建并部署；Worker 构建在 Python 基础镜像 metadata 下载 EOF 失败，运行中仍为既有镜像，未将失败隐藏为全量构建通过。 |
...
| 当前 Helm 运行态 | `helm status aiops -n observability`；`kubectl -n observability get deploy,ds,pods`；镜像标签/Pod imageID 核对 | **通过**；Helm revision 25 `STATUS=deployed`；Query API 使用 `query-api:git-865de6a`，核心 Pod Ready，图谱迁移 Job Complete。Worker/Gateway 仍使用既有镜像，Worker 新源码未因 Python 基础镜像 EOF 完成镜像级验证。 |
...
| HugeGraph scope 分页、索引边回退与自环过滤 | `cd ai-apm-query-go && GOCACHE=/tmp/aiops-go-cache go test ./... -count=1`；`go vet ./...`；Helm revision 25 migrator；只读 scope offset=0/1、K-neighbor/索引边对照 | **通过 Query 运行态**；19 个 `edgeByScope_<relation>` 索引存在；K-neighbor 对 CUSTOMIZE_STRING 顶点返回空时索引边读取成功；物化/fallback 自环被过滤，真实 RCA 图为 3/2/1。Worker TraceBuilder 源码有同样过滤，但镜像未重建。 |
...
| Ingest Event 坏时间拒绝与 WAL 回放 | `GOCACHE=/tmp/aiops-go-cache go test ./...`、`go vet ./...`（`ai-apm-ingest-go`）；Helm revision 25；真实 mTLS/API-key 事件请求与 ClickHouse 查询 | **通过**；`TestValidateEventBatchRejectsMalformedTimestampBeforeWAL` 覆盖合法 UTC、ISO `T...Z` 和错误桶；坏事件 HTTP 400、同 marker 合法事件 HTTP 202，ClickHouse `event_rows=1` 且 tenant/cluster/source/message 匹配，`events_backend_failed_total=0`；本机验证数据 WAL 已重建，未使用 fixture 冒充回放证据。 |
| RCA V2 真实 Run 与证据边界（历史 Run，已由下方最新 Run 替代） | 在冻结 `ai_runs` 窗口内，正式 outbox→Worker→RCAEngineV2 通过签名 InternalQueryClient 读取图和五类 ToolRun，并按证据质量返回终态 | 历史 Run `c3877c9f-0c0d-4f16-9a7d-8e6c5b4d3f20`；其 8 个 ToolRun 成功、图增强成功，但当时 marker/图数据不完整，RCA 为 `status=insufficient_evidence`、Run `partial`。 | `ai-orchestrator/apps/investigation.py`；`rca_engine/{engine.py,runtime.py}` | **历史证据** | 仅保留作为修复前对照，不代表当前 DeepFlow、Graph 或 RCA 运行态；当前结论以本节最新 Run 和 validator 行为为准。 |
...
| 真实 marker 全域观测与 RCA validator（最新） | 同一 marker 必须在 metrics/logs/events/DeepFlow/dependency 读到真实数据，并满足 RCA confirmed 合同 | `validate-observability-evidence.sh --marker aiops-real-20260901T132052Z-7767`：metrics/logs/Kubernetes events/DeepFlow/dependency 均 PASS（DeepFlow 80 flow 行、1 个 trace ID、1 个唯一 span ID；依赖边 2），RCA FAIL：冻结时间窗不完整、独立证据类别不足、final graph context 缺失、propagation path 缺失、root score 与 deterministic score 不一致。 | `/tmp/aiops-evidence-report-final.json`；`deploy/scripts/validate-observability-evidence.sh` | **未通过（真实证据，不是 fixture）** | 当前阻断是 RCA 合同字段未形成 confirmed 结果；不能用观测域 PASS 或 graph-context 摘要替代缺失的 RCA final artifact。 |
...
| 生产部署、回滚、观测达标 | Secret/render/image digest、PDB、health/SLO、故障和回滚 evidence 完整 | Helm templates、`runtime-slo.md`、`collect-release-evidence.sh` | Secret refs、PDB、WAL PVC、release JSON | Helm/contracts/evidence script；当前 revision 25 Ready，工作区证据脚本 `publishable=true`；真实 RCA confirmed、registry 签名和 HA 仍 BLOCKED | **未验证** | 本机 readiness 和合同通过不等于生产 HA、PITR、证书轮换、完整观测或 rollback 通过。 |

## 5. AICHAT 两个自研模块复审与改进方案

### 5.1 是否真实可用

结论：**Query `ProxyChat` 与 Orchestrator `/internal/v1/chat` 已通过本机真实登录的 SSE 和同 turn transcript replay，证明两个自研模块真实接线、scope/capability 签名、持久化和幂等可用；当前 Helm revision 25 仍使用 `LLM_MOCK=true`，生产真实模型能力仍未验证。**
...
本轮本机证据是完成真实登录/scope 后 HTTP 200、20 个 SSE event（含 1 个 done、0 个 error）；同一 `session_id+turn_id` 重试 HTTP 200 且仅 replay `done`，证明不是“只有接口定义”。当前 revision 25 deployment 的 `LLM_MOCK=true`，且没有真实 Provider key，不能把 deterministic/mock 输出认定为真实模型可用；Query persistence failure、Orchestrator queue helper、AICHAT SSE 脱敏错误边界、Investigation/RCA 错误净化、ToolRun 最终围栏、commit-error 处理和 lease/dispatch fencing 已分别通过 Go/Python 与源码断言。
...
- **AICHAT 流队列背压与错误脱敏：** `ai-orchestrator/main.py` 使用 `CHAT_STREAM_QUEUE_MAXSIZE=64` 的有界队列，入队在 `queue.Full` 时等待并响应断开事件；`_chat_stream_error_event` 统一隐藏 Provider/SQL/传输异常文本，仅返回 `CHAT_BACKEND_ERROR`；新镜像 compile/helper assertion 通过，定向 AICHAT/Worker 测试 15 passed，当前 Python 隔离全量 1229 passed、1 skipped、3 deselected、2 warnings。
...
本轮复审确认并修复了 P0-TOOL-03 的降级分支及 commit-error 缺陷：事务/租约围栏或最终提交失败时旧代码会误返回或因 `quality=complete` 写入 `eligible_for_evidence=1`，存在迟到结果进入 Evidence 或审计丢失风险；该修复由历史 revision 9 回归基线持续保留，当前 revision 25 叠加 AICHAT 错误边界、Investigation/RCA 错误净化、Run lease/commit、Outbox fencing、生产组合隔离、精确探针旁路、Graph 公共错误净化、Worker GraphSyncRuntime、scope 分页、索引边回退、自环过滤、IntentEngine 动态时间窗、Ingest Trace/Event 校验、RCA 证据窗口边界和发布工作区审计规则。当前未发现仍由代码和本机证据共同证明的越权、跨租户写入、不可逆数据破坏或核心服务必现不可用。以下 P1 项仍足以阻断生产发布。
...
- **证据：** 当前 Query/Graph 修复使用提交 `865de6a`，Query 镜像 `query-api:git-865de6a` 已在 Helm revision 25 Ready；合同、架构、Helm lint、diff check 和基础 validator 门禁通过。`collect-release-evidence.sh` 可返回 `working_tree_dirty=false,publishable=true`，但 Python Worker 完整镜像重建因基础镜像源 EOF 未完成，本机也没有 registry immutable digest/signature，不能把本机 tag 视为可审计发布物。
...
- **证据：** 本轮真实 marker `aiops-real-20260901T132052Z-7767` 已在 ClickHouse Trace、VM、VLogs、Kubernetes Event API 和 DeepFlow 读回；DeepFlow 为 80 条 flow 行/1 个 trace ID/1 个唯一 span ID，AIOps Trace SoT 为 80 行/80 个唯一 span ID/2 个服务，依赖边查询有 2 条跨服务边。最新真实只读 Run `b766233a-60d7-4476-83a3-7e657261da2a` 沿正式 outbox→Worker→RCAEngineV2 执行：9 个 ToolRun 均成功、7 条 evidence，图上下文为 3 vertices/2 edges/1 propagation path，`graph_enhanced=true`、`graph_partial=false`、`graph_stale=false`；但 Run 仍为 `partial/insufficient_evidence`、confidence=0。全域 validator 的 metrics/logs/events/DeepFlow/dependency 域均 PASS，RCA 合同仍 FAIL（时间窗不完整、独立证据类别不足、final graph context/path/确定性 root score 缺失）。没有把真实观测行或 Graph 摘要伪称 confirmed RCA。
- **触发/影响：** 生产数据源认证或租户映射错误时，根因结果只能 partial；若 UI 忽略 quality，可能误导处置。
- **根因：** 复审确认的代码问题是：`service_topology` 物化边和 parent/sequence fallback 未过滤自环、TraceBuilder 可能写入自环，以及 HugeGraph 对 CUSTOMIZE_STRING 顶点的 K-neighbor 返回空时没有索引边回退；这些问题已分别在 `865de6a`、`8c73f02`、`7f80889` 修复并在 Query 运行态验证。当前剩余阻断不是数据源完全不可用，而是 RCA 证据合同没有从真实候选证据生成 confirmed final context、bounded path 和与 deterministic score 相等的 root score；因此 fail-closed 返回 `partial/insufficient_evidence`。
- **整改实现：** Query topology 现在先读取跨服务物化边，再使用 parent-span/sequence fallback；HugeGraph Repository 在 K-neighbor 空结果时按 tenant/cluster/source 使用索引端点构造有界子图；SQL、TraceBuilder 和 fallback 均跳过自环。Query 镜像 `git-865de6a` 已部署，最终真实 Run 图从空边改善为 3 vertices/2 edges/1 path。观测入口继续保留 `NO_DATA` 与 `BACKEND_UNAVAILABLE` 的不同语义，事件入口在 WAL 前校验时间；validator 仍要求 confirmed RCA 字段，不用 fixture、零计数或单域 PASS 替代。Worker 自环过滤源码已提交，但镜像重建因 Python 基础镜像 EOF 未完成。
...
- **触发/影响：** 图高扇出、重启或恢复后 RCA 查询超时/空图，根因排序和传播路径不可信；若 Worker 与 Query 镜像不是同一候选版本，自环过滤和索引边回退行为可能不一致。
...
- **测试与结果：** `tests/test_p19_chat_ingress.py:263-283` 注入 `provider api_key=super-secret host=10.0.0.7` 异常；断言响应含 `CHAT_BACKEND_ERROR` 且不含密钥、地址或原始异常，AICHAT/Worker 定向测试 **15 passed**，当前隔离 Python 全量 **1229 passed, 1 skipped, 3 deselected, 2 warnings**。
...
- **测试与结果：** `test_investigation_runtime.py`、`test_rca_engine_v2_contract.py`、`test_investigation_worker_security.py` 和 Orchestrator 路由/错误边界定向测试共 37 项通过；Python 隔离全量为 `1229 passed, 1 skipped, 3 deselected, 2 warnings`；`compileall`、生产架构合同、部署合同通过；Worker/Gateway 容器内边界断言为历史基线，Worker 本轮未完成新镜像重建。
...
- **验证证据：** `test_worker_lifespan_starts_canonical_graph_reconcile`、Query Go 图谱单测和全量 `go vet` 通过；Helm revision 25 的 graph-schema-migrator Job 成功创建 19 个边索引；真实 scope 顶点/边 offset=0/1 查询成功；`verify-kubernetes-graph.sh --since 10m` 通过；MySQL 真实记录显示 Kubernetes generation=1/2 成功并完成代际标记。`7f80889`、`8c73f02`、`865de6a` 的 Graph 修复已包含在当前 Query 候选；Worker 自环过滤源码来自 `8c73f02`，但镜像未完成重建。
...
> **当前门禁基线：** Query/Graph 功能代码 `865de6a`、Query 镜像 `query-api:git-865de6a`、Helm revision 25。Worker 的自环过滤源码已提交但 Python 镜像重建因基础镜像 EOF 未完成；历史 revision 3–24 的代码/合同结论仅作基线，当前运行态以 revision 25 和本报告最新真实 marker 证据为准。
...
- Helm 和合同脚本均通过；本轮所有 Go module 的 `go vet ./...` 与 `go test ./...` 均通过（沙箱 IPv6 监听限制已授权本机重试）；Orchestrator Python 隔离全量 `1229 passed, 1 skipped, 3 deselected, 2 warnings`（3 个回环测试显式排除）；前端 25 个文件/39 个测试与 Vite 构建通过。
- 本机 Helm revision 25 所有核心服务 Ready；Query API 使用 `query-api:git-865de6a`，9 个内部服务实际注入 `AIOPS_TLS_CLIENT_SAN`；Gateway/Worker `wait-for-query-api` initContainer 成功；运行态 Query NetworkPolicy 选择器已核对为 `app=query-api-http`；Action Executor 保持 `disabled/realMutation=false`，未调用任何 mutation endpoint；ClickHouse 以含前导连字符的随机密码完成探针、初始化和迁移。Worker/Gateway 镜像仍为既有版本，Worker 自环过滤源码未完成镜像级验证。
...
- Investigation Worker/RCA 的错误边界已统一：`error_safety.py` 净化 Run event/result，Graph/Evidence warning 只保留稳定数据源码，LLM/stream catch 不返回异常原文；37 项定向测试、Python 全量 `1229 passed`、ARCH-333–347 合同及历史容器断言通过，P1-10 已关闭，不再作为当前代码阻断。
...
- HugeGraph 代际清理已由全图 `limit=100000`/1.5 秒交互读取改为租户/集群/source scope offset 分页和独立维护 client；当前 revision 25 的 graph-schema-migrator 创建 19 个 `edgeByScope_<relation>` 索引，真实 offset=0/1 顶点/边查询、只读 Graph 恢复观察及 `verify-kubernetes-graph.sh` 均通过。`7f80889` 新增 CUSTOMIZE_STRING 索引边回退，`8c73f02`/`865de6a` 过滤物化/fallback 自环；候选 digest、p95 和跨节点恢复仍未验证。
...
- Ingest Trace SoT `time_bucket` 已修复：`ai-apm-ingest-go/internal/tracesink/clickhouse_span_sink.go:242-270` 现在为每个 span 序列化 UTC 五分钟桶，避免 ClickHouse `trace_spans` 必填列缺失导致 400；`GOCACHE=/tmp/aiops-go-cache go test ./...` 全量通过，revision 25 真实 mTLS/API-key marker HTTP 200，ClickHouse 读回 1 行且时间桶、date、tenant、cluster 均匹配。
- Ingest Event WAL 阻塞已修复：revision 25 的 ingest pipeline 对非法 ISO `T...Z` 事件在进入 WAL 前返回 HTTP 400，合法 15 列事件 HTTP 202 并在 ClickHouse `k8s_events` 读回，tenant/cluster/source/message 均匹配，`ai_ingest_events_backend_failed_total=0`。`go test ./...`、`go vet ./...` 与新增回归测试通过。
- RCA 真实链路已复核：最新 Run `b766233a-60d7-4476-83a3-7e657261da2a` 由正式 outbox 派发，9 个 ToolRun 均成功、7 条 evidence，图上下文为 3 vertices/2 edges/1 path，`graph_enhanced=true` 且非 stale/partial；真实 marker 的 metrics/logs/events/DeepFlow/dependency 域均有读数，但终态仍是 `partial/insufficient_evidence`、confidence=0，没有伪造 confirmed 根因。全域 validator 的 RCA 合同仍失败，P1-03 继续阻断。
- Query 拓扑与 HugeGraph 回退修复已复核：`GlobalEdgesWithTraceFallback` 过滤物化/fallback 自环，`EdgesForVertex` 在 CUSTOMIZE_STRING K-neighbor 空结果时读取索引边；Query 镜像 `git-865de6a` 部署后真实 Run 从空边图改善为 2 条跨服务边，HugeGraph 当前 active self-loop=0。Worker TraceBuilder 源码同样过滤自环，但对应 Python 镜像未能因基础镜像 EOF 重建，镜像级验证未通过。
...
- P1-03：Trace 明细、事件 WAL、metrics/logs、Kubernetes Event、DeepFlow 和依赖边的真实 marker 读回均已通过；DeepFlow 统计为 80 flow 行/1 trace ID/1 唯一 span ID，依赖边 2 条。最新真实 RCA Run 图增强/ToolRun 审计已通过（3 vertices/2 edges/1 path、9 个 ToolRun、7 条 evidence），但确定性结果仍为 `partial/insufficient_evidence`。全域 validator 仍不能 PASS，当前明确失败为冻结时间窗、独立证据类别、final graph context/path 和 root score 合同；不能把单域观测 PASS、历史 canary、零计数或 fixture 外推为 confirmed RCA。
...
本轮修订覆盖上一轮 revision 19/早期 revision 2 的运行态描述：当前运行态以 Helm revision 25、Query/Graph 功能代码 `865de6a`、Query 镜像 `query-api:git-865de6a` 以及真实 marker 证据为准；旧 revision/标签仅作为历史基线，历史 canary、fixture 和零计数不作为当前真实观测证据。报告文档提交不改变运行镜像。Investigation Worker GraphSyncRuntime 生命周期、HugeGraph scope 分页/索引边回退、自环过滤、RCA 错误边界、Graph/Evidence warning code、Query Graph 公共错误净化、生产组合隔离、Gateway/Worker 探针路径、IntentEngine 动态时间窗、Ingest Trace SoT `time_bucket`、事件坏时间拒绝、RCA 证据窗口闭区间和发布工作区审计规则已通过代码、测试、迁移和真实运行验证；ClickHouse 前导连字符密码故障已真实复现并修复；Graph 200k/1M 写入、200k alias projection、7 个只读门禁和资源快照仍明确是不压测证据（heap-used 未采集）。当前真实 marker 的观测域和依赖图已读回，但 RCA confirmed 合同、Worker 新镜像、生产 Provider、HA/PITR、digest/signature 仍阻断发布。
