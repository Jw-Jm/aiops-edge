# AIOps UI V2 实际渲染与编码对照

本目录是已确认产品架构的高保真视觉合同。所有 PNG 都由同一份可运行 HTML 在真实 Chromium 中渲染，不是手绘概念图。页面中的集群、资源、指标和风险均为**设计示例数据**，只用于表达信息结构和视觉状态，编码时必须替换为权威接口数据。

可运行原型：[aiops-ui-prototype.html](./aiops-ui-prototype.html)

打开原型时使用 `?view=` 切换页面：`login`、`platform`、`cluster`、`resources`、`resource-detail`、`observe`、`investigation`、`actions`、`graph`、`reports`、`admin`。

## 1. 页面渲染图

### 登录

![登录页](./00-login-1440.png)

编码要点：左侧只表达真实产品能力；右侧登录表单保持单任务；禁止虚构告警数量、客户数量或健康状态。

### 生产云平台健康

![生产云平台健康](./01-platform-overview-1440.png)

编码要点：平台状态与依据是视觉主语；集群四态、覆盖率分子/分母、最新数据、平台组件和最高优先级风险必须同屏可判断；本页不受活动集群影响。

### 集群详细总览

![集群详细总览](./02-cluster-overview-1440.png)

编码要点：先显示集群状态依据和问题队列，再用同等视觉权重展示容器资源与 KubeVirt 虚拟机；运行支撑作为依赖带呈现，严重问题需提升到问题队列。

### 资源目录

![资源目录](./03-resource-catalog-1440.png)

编码要点：资源分为容器、KubeVirt、运行支撑三个工作组；搜索与 Namespace 仅为本页局部筛选；1440px 使用筛选、结果、快速详情三段结构。

### 资源详情

![资源详情](./04-resource-detail-1440.png)

编码要点：身份和健康依据位于顶部；证据是主工作区；关系、影响和 capability 位于右侧；技术值必须可完整查看和复制。

### 观测工作区

![观测工作区](./05-observe-1440.png)

编码要点：以问题和证据模式组织，不以数据源产品名组织；所有图表显示单位、时间范围、来源；右侧时间线可直接进入调查。

### 调查工作区

![调查工作区](./06-investigation-1440.png)

编码要点：冻结 Scope 独立成带；证据与主故障链占主要宽度；结论、候选假设、反证、缺失证据和动作建议集中在右侧。

### 处置工作区

![处置工作区](./07-actions-workspace-1440.png)

编码要点：先展示状态管线，再展示动作队列和完整详情；预检、审批、执行、验证和回滚条件必须连续可追溯。

### 资源关系图谱

![资源关系图谱](./08-knowledge-graph-1440.png)

编码要点：容器与 KubeVirt 是同级泳道；运行支撑位于依赖层；聚合节点必须显示总数和异常数；右侧检查器提供完整内容，画布节点保持有边界。

### 报告

![报告](./09-reports-1440.png)

编码要点：报告以真实资源事件为主语，显式展示影响、证据、处置和验证结果；历史业务语义只能作为标签。

### 系统管理

![系统管理](./10-admin-1440.png)

编码要点：接入状态和健康状态分列，不能把“已纳管”显示成“健康”；设置导航与日常运维导航保持视觉区分。

## 2. 响应式实渲染

### 1280×720 平台总览

![1280 平台总览](./11-platform-overview-1280.png)

内容画布允许纵向滚动以访问完整内容；禁止页面级横向溢出。

### 1024×768 资源目录

![1024 资源目录](./12-resource-catalog-1024.png)

筛选栏收敛为“筛选”按钮，资源详情进入独立页面或 Drawer，结果区获得完整宽度。

### 1024×768 知识图谱

![1024 知识图谱](./13-knowledge-graph-1024.png)

检查器收敛为按钮/Drawer，画布完整保留；节点总量和当前加载量仍需可见。

## 3. 编码视觉合同

### 页面框架

- 导航轨：1440px 视口下 88px；1280px 及以下 72px。
- 工作区顶栏：64px；只有真实集群选择、时间、通知和用户入口。
- 页面内容：12 栏逻辑网格，外边距 28px；1280px 及以下收敛到 20px。
- 页面级搜索不存在；资源搜索只能放在集群内资源目录。
- 页面标题区只保留范围说明、最后更新时间和最多两个主动作。

### 基础尺寸

- 页面标题 22px，区域标题 15–16px，正文 13–14px，辅助文字 10–12px。
- 卡片圆角 10px，控件圆角 7–8px，主要点击高度不低于 36px。
- 页面主间距 24px，区域间距 14–20px，卡片内间距 12–16px。
- 卡片使用边框建立结构，不使用装饰性阴影或大面积渐变。

### 建议组件边界

- `AppFrame`：导航轨、顶栏、响应式内容边界。
- `ClusterNavigator`：选择真实集群、Scope 提交与回读、成功后导航。
- `PlatformHealthBand`：平台状态依据、四态分布、覆盖率、最新数据。
- `ClusterHealthMatrix`、`PriorityRiskQueue`、`PlatformComponentStatus`。
- `ClusterOverviewHeader`、`CarrierPlanePanel`、`RuntimeSupportStrip`。
- `ResourceCatalogFilters`、`ResourceResultList`、`ResourceQuickInspector`。
- `ResourceIdentityBand`、`EvidenceWorkspace`、`RelationContextPanel`。
- `InvestigationScopeStrip`、`EvidenceTimeline`、`HypothesisPanel`。
- `ActionPipeline`、`ActionQueue`、`ActionInspector`。
- `KnowledgeGraphWorkspace`、`GraphToolbar`、`GraphInspector`、`EquivalentRelationList`。
- `BoundedDataRegion`：统一 loading、empty、error、partial、stale、forbidden 状态及边界行为。

### 完整显示与不超限

- 主标题、资源名称、状态原因、风险、错误和动作结果必须有完整查看路径。
- 短文本换行；长描述提供“展开全部”；Tooltip 不能是唯一完整内容入口。
- UID、镜像和长标签允许容器内断词，并提供完整复制。
- 页面不横向滚动；宽表仅允许在自身有边界的容器内滚动，或将辅助字段移动到详情。
- 大列表必须分页或虚拟滚动，并显示匹配总数与当前范围。
- 图谱 80 节点/200 边仅是首屏渲染预算；超出部分使用聚合、按页展开和等价关系列表，不能静默截断。
- 所有 Drawer、Dialog、Popover 都要设置视口内最大宽高、内部滚动和始终可见的关闭入口。

### 数据和状态

- 健康只显示健康、降级、严重、未知，并同时使用颜色、图标和文字。
- UI 不计算或展示 0–100 综合健康分数。
- 平台聚合、集群概览、资源详情使用不同 query key 和缓存边界。
- 每个数据区必须实现正常、加载、空、错误、部分、陈旧、无权限七种呈现。
- 图中的内容为视觉示例；实际编码禁止用示例数据作为接口失败时的回退。

## 4. 使用方式

编码时先按本目录建立壳层和基础 Token，再逐页对照 PNG 完成组件结构。每页在 1440×900、1280×720、1024×768 下截图比较：检查结构层级、间距、换行、局部滚动、浮层边界、状态语义和内容完整入口。像素差异可以来自真实数据长度，但不得改变本文定义的信息优先级与组件边界。
