...
| ongrid | 自研现状 | 差距 |
|---|---|---|
| 告警调查 investigator（告警上"调查"按钮，点击触发 RCA）| 仅手动触发 | 🔴 |
...
**A1. 告警调查（investigator，手动按钮触发）**
- 告警事件详情页/列表增加"调查"按钮 → 点击后后台触发 RCA 分析（异步，不阻塞）
- 分析结果（根因假设/证据链）关联到事件，前端 incident 详情展示
- 复用现有确定性 + 假设 RCA 引擎；**程序不自动触发调查**，仅用户点击按钮触发
- 调查状态（进行中/完成/失败）+ 结果时间线展示
...
**A5. 告警恢复（AI 方案 → 审批 → 执行，非自动）**
- 调查后 **AI 生成恢复方案**（操作步骤/影响面/风险说明）
- 方案提交 → **人员审核**（审批人 = admin + 用户管理中配置的可审审批人），同意后才执行恢复操作
- **白名单安全边界（可配置）**：默认允许「重启/重调度 pod、扩容、回滚、重启服务」；禁止「删除资源、清数据、生产关键库变更」；白名单可在设置中调整
- 执行记录审计（operator/方案/审批人/动作/结果）
- 补充：恢复操作后人工确认事件 resolve（或经审批的恢复成功标记 resolve）
