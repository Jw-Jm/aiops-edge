...
require_contains 'name: rag-bootstrap' "${tmp_dir}/validation.yaml" 'ai-orchestrator does not bootstrap the Chroma collections before runtime'
require_contains 'command: ["python", "-m", "rag_bootstrap"]' "${tmp_dir}/validation.yaml" 'ai-orchestrator RAG bootstrap command is missing'
require_contains 'ENV HOME=/home/aiops' "${repo_root}/ai-orchestrator/Dockerfile" 'ai-orchestrator does not expose the offline model cache to its runtime user'
